...
if(substr($jver, 0, 3) == '1.5') {
	...
	$params =  & $app->getParams('com_dreamworkgallery');
}
...
$module_path = JPATH_SITE . DS . $pic_path . DS;
...
if (!function_exists('getCatResDreamW')) {
function getCatResDreamW($params, $params_comg) {
...
if (!function_exists('getItemsCatDreamW')) {
	function getItemsCatDreamW($cat_id) {
...
if (!function_exists('create_galldreamworkgallery_xml_file')) {
function create_galldreamworkgallery_xml_file($params, &$catppv_id, $params_comg)
{
	$pic_path = $params_comg->get('pic_path', 'images/dreamworkgallery/gallery');
	if (1 == trim($params->get( 'catppv_flag', '2' ))) {
		$catppv_id .= md5(print_r($params, true));
	}

	$module_path_swf = JPATH_SITE . DS . 'components'.DS.'com_dreamworkgallery'.DS.'sb'.DS.'modules'.DS;
	$module_path = JPATH_SITE . DS . $pic_path . DS;
	if (!file_exists($module_path . $catppv_id . '.swf') ) {
		copy($module_path_swf . 'mod_xmlswf_gallery_dreamwork.swf', $module_path . $catppv_id . '.swf');
	...
	$cat_q_res = getCatResDreamW($params, $params_comg);
...
foreach ($cat_q_res as $curr_category)
{
	...
	$ret_arr = write_galdreamworkgallery_xml_data($curr_category->id, $curr_category->id, $params, $params_comg);
	if ($ret_arr['flag']) {
$xml_data_data .= '<category>';
...
if (!function_exists('write_galdreamworkgallery_xml_data')) {
function write_galdreamworkgallery_xml_data($cat_name, $cat_id, $params, $params_comg)
{
	...
	$xml_data = '';
	$prod_q_res = getItemsCatDreamW($cat_id);

	foreach ($prod_q_res as $curr_product) {
		if ($params->get( 'showdesc', '1' ) == 1) {
			$description = '<![CDATA[<font color="'.GetHColor($params, 'titlefontColor', 'FFFFFF').'" size="'.$params->get( 'descfontsize', '15').'" >'.$curr_product->descr.'</font>]]>';
		}
        else {
			$description =	'';
		}
...
$xml_fname    = 'dreamworkgallery';
$catppv_id = 'xml/' . $xml_fname;
$module_path_swf = JPATH_SITE . DS . 'components'.DS.'com_dreamworkgallery'.DS.'sb'.DS.'modules'.DS;

if (!is_dir($module_path . DS . 'xml')) {
	mkdir($module_path .  DS . 'xml', 0777);
}

create_galldreamworkgallery_xml_file($params, $catppv_id, $params_comg);
?>
<div align="center" id="com">
<script src="<?php echo JURI::root()?>components/com_dreamworkgallery/sb/AC_RunActiveContent.js" type="text/javascript"></script>
<script type="text/javascript">
	...
	if (typeof AC_FL_RunContent == "undefined") {
		...
		AC_FL_RunContent(
			...
			'src', '<?php echo JURI::root() . $pic_path.'/' . $catppv_id; ?>',
			'quality', 'high',
			...
			'name', 'gallery',
			'menu', 'true',
			'mobile', '<?php echo JURI::root() . $pic_path.'/'. $catppv_id; ?>.xml',
			'allowFullScreen', 'true',
			'flashVars', 'data=<?php echo JURI::root() . $pic_path.'/'. $catppv_id; ?>.xml',
			'allowScriptAccess','sameDomain',
			'movie', '<?php echo JURI::root() . $pic_path.'/' . $catppv_id; ?>',
			'salign', ''
			); //end AC code
	}
</script>
