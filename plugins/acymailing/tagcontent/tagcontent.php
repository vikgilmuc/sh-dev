	...
	<?php
	 ...
	 function _replaceContent(&$results,$i){
		...
		if(!empty($tag->lang)){
			...
			if(!empty($langid)){
				$query = "SELECT reference_field, value FROM ".(ACYMAILING_J16 ? '`#__falang_content`' : '`#__jf_content`')." WHERE `published` = 1 AND `reference_table` = 'content' AND `language_id` = $langid AND `reference_id` = ".$tag->id;
				$db->setQuery($query);
